...
import type { Pessoa, Produto, PaginatedResponse, OrcamentoItem } from '~/types'
...
export function useOrcamentoForm() {
  ...
  function defaultState() {
    return {
      ...
      itens: [{ produto_id: null, descricao: '', quantidade: 1, valor_unitario: 0, desconto: 0 }] as OrcamentoItem[]
    }
  }

  function addItem(state: { itens: OrcamentoItem[] }) {
    state.itens.push({ produto_id: null, descricao: '', quantidade: 1, valor_unitario: 0, desconto: 0 })
  }

  function removeItem(state: { itens: OrcamentoItem[] }, index: number) {
    if (state.itens.length > 1) {
      state.itens.splice(index, 1)
    }
  }

  function onProdutoSelect(index: number, produtoId: number | null, state: { itens: OrcamentoItem[] }) {
    if (!produtoId) return
  ...
  function calcTotalItens(state: { itens: OrcamentoItem[] }) {
    return state.itens.reduce((sum: number, item: OrcamentoItem) => {
      return sum + (item.quantidade * item.valor_unitario) - (item.desconto || 0)
    }, 0)
  }

  function calcTotalGeral(state: { itens: OrcamentoItem[], desconto: number | undefined }) {
  ...
  function populateFromEntity(state: ReturnType<typeof defaultState>, entity: {
    ...
    state.validade = entity.validade || ''
...
export type { Schema as OrcamentoSchema }
