...
import type { CompanyInvoice, PaginatedResponse, Period, Range } from '~/types'
...
const { data, status, refresh } = useApi<PaginatedResponse<CompanyInvoice>>('/company-invoices', {
  lazy: true,
  query: queryParams
})
...
}>('/company-invoices/dashboard', { lazy: true })
...
async function markAsPaid(invoice: CompanyInvoice) {
  try {
    await put(`/company-invoices/${invoice.id}`, { status: 'paid' })
...
const columns: TableColumn<CompanyInvoice>[] = [
  {
    id: 'empresa',
    header: 'Empresa',
    cell: ({ row }) => h('div', [
      h('p', { class: 'font-medium text-highlighted' }, row.original.company?.fantasia || row.original.company?.razao_social || '—'),
  ...
  },
  {
    id: 'plano',
    header: 'Plano',
    cell: ({ row }) => row.original.office_plan?.name || h('span', { class: 'text-muted' }, '—')
  },
  {
    accessorKey: 'amount',
    header: 'Valor',
...
<template>
  <UDashboardPanel id="escritorio-cobrancas">
    <template #header>
      <UDashboardNavbar title="Cobranças">
        ...
        <template #right>
          <CobrancasGenerateModal @generated="refresh(); refreshDashboard()" />
        </template>
      </UDashboardNavbar>
    ...
    <template #body>
      ...
      <div class="flex items-center justify-between gap-3 border-t border-default pt-4 mt-auto">
        <div class="text-sm text-muted">
          ...
          {{ table?.tableApi?.getFilteredRowModel().rows.length || 0 }} fatura(s) selecionada(s).
        </div>
        <UPagination
