...
async function onSubmit(event: FormSubmitEvent<PedidoSchema>) {
  loading.value = true
  try {
    const pedido = await post<Pedido>('/pedidos', event.data)
    toast.add({ title: 'Pedido criado', description: `Pedido #${pedido.numero} criado com sucesso.`, color: 'success' })
    router.push(`/comercial/pedidos/${pedido.id}`)
  } catch (error) {
    toast.add({ title: 'Erro', description: extractMessage(error) || 'Erro ao criar pedido.', color: 'error' })
  } finally {
    loading.value = false
  }
}
...
<template>
  <UDashboardPanel id="pedido-novo">
    <template #header>
