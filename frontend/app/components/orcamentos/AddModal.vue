...
const state = reactive<OrcamentoState>({
  pessoa_id: null,
...
async function onSubmit(event: FormSubmitEvent<Schema>) {
  loading.value = true
  try {
    await post('/orcamentos', event.data)
    toast.add({ title: 'Orçamento criado', color: 'success' })
    open.value = false
    resetForm()
    emit('created')
  } catch (error) {
...
<template>
  <UModal
    ...
    description="Criar um novo orçamento com itens"
    :ui="{ content: 'w-full sm:max-w-5xl', footer: 'justify-end' }"
  >
    <UButton label="Novo Orçamento" icon="i-lucide-plus" />
