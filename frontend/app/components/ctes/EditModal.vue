...
watch(open, (val) => {
  if (val) {
    Object.assign(state, {
      natureza_operacao: props.cte.natureza_operacao || 'Prestação de Serviço de Transporte',
      cfop: props.cte.cfop || '',
      modal: props.cte.modal || 1,
      data_emissao: props.cte.data_emissao?.split('T')[0] || new Date().toISOString().split('T')[0],
      remetente_id: props.cte.remetente_id,
      destinatario_id: props.cte.destinatario_id,
      expedidor_id: props.cte.expedidor_id || null,
      recebedor_id: props.cte.recebedor_id || null,
      tomador_id: props.cte.tomador_id || null,
      tomador_tipo: props.cte.tomador_tipo || 0,
      valor_servico: Number(props.cte.valor_servico) || 0,
      valor_receber: Number(props.cte.valor_receber) || 0,
      valor_icms: Number(props.cte.valor_icms) || 0,
      valor_total: Number(props.cte.valor_total) || 0,
      uf_inicio: props.cte.uf_inicio || '',
      uf_fim: props.cte.uf_fim || '',
      informacoes_adicionais: props.cte.informacoes_adicionais || ''
    })
    nfes.value = (props.cte.nfes || []).map(n => ({ chave_nfe: n.chave_nfe }))
  }
})
...
async function onSubmit(event: FormSubmitEvent<Schema>) {
  ...
  try {
    ...
    await put(`/ctes/${props.cte.id}`, payload)
...
<template>
  <UModal
    v-model:open="open"
    title="Editar CT-e"
    description="Alterar dados do conhecimento de transporte eletrônico"
