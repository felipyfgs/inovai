...
async function onSubmit() {
  loading.value = true
  try {
    const res = await post<{ message: string, created: number }>('/company-invoices/generate-monthly', form)
    useAppToast().success(res.message)
    open.value = false
    emit('generated')
...
<template>
  <UModal
    ...
    description="Gera faturas mensais para todas as empresas ativas com assinatura."
