...
watch(open, (val) => {
  if (val) {
    Object.assign(state, {
      ...
      tipo_pessoa: props.pessoa.tipo_pessoa,
      razao_social: props.pessoa.razao_social,
      fantasia: props.pessoa.fantasia || '',
      cpf_cnpj: props.pessoa.cpf_cnpj || '',
      ie: props.pessoa.ie || '',
      ind_ie: props.pessoa.ind_ie,
      logradouro: props.pessoa.logradouro || '',
      numero: props.pessoa.numero || '',
      complemento: props.pessoa.complemento || '',
      bairro: props.pessoa.bairro || '',
      municipio: props.pessoa.municipio || '',
      municipio_ibge: props.pessoa.municipio_ibge || '',
      uf: props.pessoa.uf || '',
      cep: props.pessoa.cep || '',
      telefone: props.pessoa.telefone || '',
      celular: props.pessoa.celular || '',
      email: props.pessoa.email || '',
      observacoes: props.pessoa.observacoes || ''
    })
  }
})
...
async function onSubmit(event: FormSubmitEvent<Schema>) {
  loading.value = true
  try {
    await put(`/pessoas/${props.pessoa.id}`, event.data)
...
<template>
  <UModal
    v-model:open="open"
    title="Editar Pessoa"
    description="Alterar dados da pessoa"
